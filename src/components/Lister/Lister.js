...
const lagNyListe = () => {
    firestore.collection(COLLECTION_LISTER).add({edit: true, deleted: false})
        .then(reference => {
            reference.collection(COLLECTION_LISTE)
                .doc('dummydokument')
...
const RadIEditModus = ({listeDok}) => {
    ...
    return (
        <>
            ...
            <IconButton aria-label="angre" size="medium" color="secondary"
                        onMouseDown={() => {
...
const RadILenkeModus = ({listeDok}) => {
    ...
    return (
        <>
            ...
            <IconButton aria-label="Endre navn"
                        size="medium"
                        color="primary"
                        onClick={() => rediger(listeDok)}>
                <Edit/>
            </IconButton>
            <IconButton aria-label="Slett liste"
                        size="medium"
                        color="primary"
    ...
    return (
        <div>
            ...
            <ul className={css.content}>
                ...
                {lister && lister.docs.map(listeDok => (
                    <ListeLenke key={listeDok.id} dokument={listeDok}/>
                ))}
            </ul>
        </div>
    );
